...
export const Demo = ({ user, content, time, picture, demo }) => {
  return (
    <div
      className={
    ...
    >
      ...
      <div className="text-4xl text-dark font-bold">{demo.title}</div>
      ...
      <div className="flex flex-row mt-2 items-center">
        <Location /> <span className="mx-1">{demo.location}</span>
      </div>
      <div className="rounded-xl mt-6 overflow-hidden">
        <ReactPlayer url={demo.video} muted={true} />
      </div>

      <div className={'mt-4 p-2 w-3/4 text-left text-sm '}>{demo.description}</div>
